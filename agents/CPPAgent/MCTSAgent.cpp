...
std::vector<std::vector<Tile>> convert_board_string(const std::string& board_string, const int boardSize) {
...
MCTSAgent::MCTSAgent(const std::string& agentColour, int gameBoardSize)
    : colour(agentColour), turn(-1), boardSize(gameBoardSize) {}
...
void simulate_iterations(std::atomic<int>& iterations, MCTSNode* root, std::string colour, std::chrono::steady_clock::time_point end_time , int thread_number) {
    while (std::chrono::steady_clock::now() < end_time) {
        MCTSNode* node = root->get_child(thread_number);
        double result = node->simulate_from_node(colour);
        node->backpropagate(result);
        iterations.fetch_add(1, std::memory_order_relaxed);  // Increment safely
    }
}

void run_mcts_in_parallel(MCTSNode* root, const std::string& colour, std::chrono::steady_clock::time_point end_time, int num_threads) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        threads.push_back(std::thread(simulate_iterations, std::ref(iterations), root, colour, end_time, i));
    }
...
void MCTSAgent::makeMove(const std::string& board) {
    ...
    std::vector<std::vector<Tile>> boardState = convert_board_string(board, boardSize);

    Board brd(boardState, boardSize, "");
    MCTSNode root(brd, colour, nullptr, {-1, -1});
    root.generate_all_children_nodes();
    std::chrono::steady_clock::time_point end_time = std::chrono::steady_clock::now() + std::chrono::milliseconds(4500);
    run_mcts_in_parallel(&root, colour, end_time, root.get_children_size());

    std::pair<int, int> best_move = root.get_best_move();
    sendMessage(std::to_string(best_move.first) + "," + std::to_string(best_move.second));
}
