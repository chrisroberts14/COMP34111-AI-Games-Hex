...
std::vector<std::pair<int, int>> get_valid_moves(const std::string state, const int boardSize) {
...
std::string MCTSNode::DFSColour(int x, int y, const std::string& colour, std::set<std::pair<int, int>>& visited){
    int size = 11;
    std::vector<std::vector<Tile>>& tiles = this->state.get_state();
    ...
    while (!stack.empty()) {
        ...
        for (int i = 0; i < Tile::NEIGHBOUR_COUNT; ++i) {
            ...
            if (x_n >= 0 && x_n < size && y_n >= 0 && y_n < size) {
                if (visited.find({x_n, y_n}) == visited.end() && tiles[x_n][y_n].getColour() == colour) {
...
double MCTSNode::simulate_from_node(std::string& current_colour) {
    ...
    // The reward here is higher than is possible for other moves which guarantees that the winning move is chosen
    ...
    while (true) {
        ...
        if (winner != "") {
            ...
            return (std::exp((long double)(-0.05 * moves_taken.size())) * (this->colour == current_colour ? 1 : -1));
        }
...
MCTSNode* MCTSNode::best_child(float c) {
    ...
    for (MCTSNode* child : this->children) {
        ...
        double score = child->payoff_sum / child->visits + c * std::sqrt(2 * log_parent_visits / child->visits);
        if (score > best_score) {
            best_score = score;
            best_node = child;
        }
...
MCTSNode* MCTSNode::get_child(int index) {
    return this->children[index];
}

int MCTSNode::get_children_size() {
